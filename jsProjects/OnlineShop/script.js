...
function renderCards() {
	products.removeEventListener('click', clickHandler)
	products.innerHTML = ''
...
function renderCart() {
	products.innerHTML = ''
	products.innerHTML = `${fillTable(cartArr)} <div id="total-price">Total: &euro; ${cartArr.reduce((sum, el) => sum + (el.price * el.quantity), 0)}</div>`
}
...
function fillTable(arr) {
	...
	<div class="head" id="head-total">Total</div>`
		for (let i = 0; i < arr.length; i++) {
			data += `<div class="grid" id="title_${i}"><i class = 'fas fa-trash' data-id = ${i}></i> ${arr[i].title}</div>
	<div class="grid" id="price_${i}">&euro; ${arr[i].price}</div>
	<div class="grid" id="count_${i}">${arr[i].quantity}</div>
	<div class="grid" id="total_${i}">&euro; ${arr[i].price * arr[i].quantity}</div>`
		}

		products.addEventListener('click', clickHandler)
