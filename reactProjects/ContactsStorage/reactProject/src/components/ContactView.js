...
import { contactByIdSelector, deleteContactAction, getAllContactsAction } from '../store/contacts'
...
const ContactView = () => {
    const { id } = useParams() // hook to receive id from props.match
    ...
    return (
        ...
        <Wrapper>
            ...
            <ButtonRow>
                ...
                <RemoveButton onClick={()=>{dispatch(deleteContactAction(contact.id)); history.push('/contacts/')}}>remove</RemoveButton>
            </ButtonRow>
        </Wrapper> : <Loader/> }
        </>
    )
